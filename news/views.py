...
class HomePage(ListView):
    ...
    def get_queryset(self):
        ...
        return Post.objects.filter(published=True).order_by('-publish')[:5]
...
def trending_posts(request):
    ...
    return {'post_trending': post_trending}

def one_trending_news(request):
    category_trend = News_Category.objects.get(title='Trending News')
    one_news = Post.objects.filter(category=category_trend).order_by('-publish')[:1]
...
def buisness_news(request):
    ...
    return {'buisness_new': buisness_new}
